...
// 내부의 테스트 메서드가 끝날 때마다 트랜잭션을 Rollback한다.
// 테스트가 끝난후 DB에 데이터가 Rollback되므로 DB를 깔끔하게 관리할 수 있다.
@Transactional
@SpringBootTest
...
//  @BeforeEach 는 JUnit에서 지원하는 어노테이션이다.
...
//        3. then
        ...
        // contains() 는 뽑아낸 리스트에 "test2"가 포함되었는지 검증한다.
        assertThat(list)
                .extracting("title")
                .contains("test2");

        assertThat(list)
                .extracting(BoardListDTO::getTitle)
                .contains("test2");
    }
